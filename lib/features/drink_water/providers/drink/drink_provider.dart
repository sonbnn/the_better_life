...
class DrinkProvider extends ChangeNotifier {
  ...
  double amount = 250;
  List<HistoryDay> listHistoryDay = [];
  List<HistoryMonth> listHistoryMonth = [];
  String currentDay = '';
  ...
  void addWater() {
    ...
    progress = (amountDrinkToday / ConstantWater.recommendedMilli >= 1) ? 1.0 : amountDrinkToday / ConstantWater.recommendedMilli;
  ...
  void addHistoryDay({required String timeRef, required double amount}) async {
    listHistoryDay.insert(0,HistoryDay(time: timeRef, amount: amount));
